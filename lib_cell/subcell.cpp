...
CSubcell::CSubcell(int sizex, int sizey, int sizez, int fmesh, double xii, int _seed)
{
        ...
        // gca     = 1.475 * gca_change;
        pNa     = 1.0 / 8113.0 * 0.75e-8 * 0.96245 * (1e-10); // [cm/sec*F]
        pK      = 1.0 / 8113.0 * 1.35e-7 * 0.96245 * (1e-10); // [cm/sec*F]
}


void CSubcell::init(double initci, double initcj, double initnai, double gca_change, std::string serca_map_file)
{       
        gca     = 1.475 * gca_change;
	...
	// serca_map_file = "pool_serca/RA.txt";
        std::cout << serca_map_file << std::endl;
        // To load SERCA pattern
        std::ifstream inSERCA(serca_map_file.c_str());
        if(inSERCA.fail()){
                cout << "SERRCA file not exist" << endl;
                exit(1);
        }
        std::string SERCAline;
        int serca_id   = 0;
        double serca_value = 0;
        while (std::getline(inSERCA, SERCAline))
        {
                // each CRU at least has 1 CSQ
                std::stringstream ssSERCA(SERCAline);
                ssSERCA >> serca_value;
                set_serca(serca_id, serca_value);
                ++serca_id;
        }       
